...
var rasterPattern = function(color) {
	c = new fabric.StaticCanvas();
...
for (var i = 0; i < hours.length; i++) {
	...
	canvas.add(new fabric.Text(tijd_tekst, {
        left: tijd_x,
        top: tijd_y,
        fontSize: font_size,
        fontFamily: 'Verdana',
...
for (var d = 0; d < items.dates.length; d++) {
	...
	for (var col = 0; col < day.resources.length; col++) {
		var colX = outerX + labelWidth + 0.5 + (colWidth * col);
		// Kolom label
		var resource = day.resources[col];
		var resource_label = resource.key;
		var resource_y = outerY + 2;
		var resource_width = colWidth - 2;
		var resource_height = rowHeight - 2;
		canvas.add(new fabric.Text(resource_label, {
	        left: colX + 1.5,
	        top: resource_y,
		...
		for (var s = 0; s < resource.schedule.length; s++) {
			var scheduleItem = resource.schedule[s];
			var scheduleY = timeStringToY(scheduleItem.begin) + 0.5;
			var scheduleHeight = timeStringToY(scheduleItem.end) + 0.5 - scheduleY;
			...
			canvas.add(new fabric.Rect({
				...
				fill: rasterPattern(colorFromActivities(scheduleItem.activities)),
				strokeWidth: 1
			}));

			canvas.add(new fabric.Textbox(scheduleItem.activities.join(), {
		        left: colX + 2,
		        top: scheduleY + 2,
				width: colWidth - 2,
...
//				y: 0.5,
//				width: colWidth,
//				height: timeStringToY(appointment.end) + 0.5 - appointmentY,
//				stroke: '#5a6d84',
//				strokeWidth: 0.5,
//				fill: '#C0C0FF',
//				cornerRadius: 4
