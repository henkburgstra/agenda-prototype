...
for (var d = 0; d < items.dates.length; d++) {
	...
	for (var col = 0; col < day.resources.length; col++) {
		...
		var l = new createjs.Text(resource_label, "14px Verdana", "black");
		l.x = colX + 1.5;
		l.y = resource_y;
		stage.addChild(l);
		// Rooster items
		for (var s = 0; s < resource.schedule.length; s++) {
			...
			r = new createjs.Shape();
			r.graphics
				.beginBitmapFill(rasterCanvas(colorFromActivities(scheduleItem.activities)))
				.rect(colX, scheduleY, colWidth, scheduleHeight);
			stage.addChild(r);

			var l = new createjs.Text(scheduleItem.activities.join(), "11px Verdana", "#5a6d84");
			l.x = colX + 2;
			l.y = scheduleY + 2;
			l.lineWidth = colWidth - 2;
			l.mask = r;
			stage.addChild(l);
		...
		for (var a = 0; a < resource.items.length; a++) {
			...
			a.graphics
				...
				.beginStroke("#5a6d84")
				.beginFill("#C0C0FF")
				.rect(0.5, 0.5, colWidth, timeStringToY(appointment.end) + 0.5 - appointmentY);
